...
// pass of assembler
 ...
 int main( int argc, char *argv[])
 {
   ...
   std::ofstream outFile;   // output obj file 
	...
	{
		outFile << std::hex << std::setw(8) << std::setfill('0') <<(*it).machineCode << std::endl;
	} //*/
 ...
 // in pass one we will find all the labels and assign any data to their places in 
 // memory
 void PassOne(std::list<std::string> &sourceCode, std::list<label> &addressTable, 
				std::list<machine> &machineCode)
 {
	 int globalPointer = 0;
	 label lbl;
	 ...
	 // code and see if it is a label, if so put it in the label table
	 ...
	 while(!flag2 && it != sourceCode.end())	
	{
		...
		flag2 = (*it) == ".data";
		if((*it) == ".text")        // if the string is .text move to the next line of code
		{
		  ++it;
		  continue;
		}
		
		// if the string contains a : then it is a label and we will place it into the address table
	...
	// we will iterate through the .data section and place labels in the address table and
	// the words we want placed in memory will be put into the machine code
	while( it != sourceCode.end())
	{
		...
		++itr;
		// if .word we will put the next item into memory at the current address
		if((*itr) == ".word")
		{
			// move to the next chunk of code
			++itr;
		...
		// if the word is .space we will place n sets of 0 on the back of the machine code
		else if((*itr) == ".space")
		{
			++itr;
 ...
 // in pass two we will generate the machine code for every line of code
 void PassTwo(std::list<std::string> &sourceCode, std::list<label> &addressTable, 
				std::list<machine> &machineCode)
 {
	int globalPointer = 0;
	...
	for(std::list<std::string>::iterator it = sourceCode.begin(); 
		it != sourceCode.end(); ++it)
		{
			...
			else if((*itr).find_first_of(':') != std::string::npos ) continue;
			...
			{  
			  ...
			  { // with 1 argument
				if(isOneArg(*itr))
				{
				  ...
				  mc = mc | (functCode(*itr));       // put in function code
				  ++itr;
				  mc = mc | (regCode(*itr) << 11);   // put in register code
			  ...
			  else if(isIType(*itr))
			  {
				...
				if((*itr) == "addiu")
				{
				  ...
				  ++itr;
				  mc = mc | (atoi((*itr).c_str()));    // load in offset
				}
				...
				else if((*itr) == "beq" || (*itr) == "bne")
				{
				  ...
				  mc = mc | (regCode(*itr) << 16);  // put in rt
				  ++itr;
				  // we will go search the address table for the address of the label used
				  for(std::list<label>::iterator itt = addressTable.begin(); 
				    itt != addressTable.end(); ++itt)
				  {
		    	    if((*itr) == (*itt).labelName ) 
				    {
					  // we will subtract the global pointer from the address, 
					  // masking the first 16 bits in case of negative offset
					  int mask = 0x0000FFFF;
				...
				else // if lw and sw
				{
				  ...
				  mc = mc | (regCode(*itr) << 16);    // put in rt code
 				  ++itr;
				  // next we will search the address table in the case of a label being used
				  for(std::list<label>::iterator itt = addressTable.begin(); 
				    itt != addressTable.end(); ++itt)
				  {
		    	    if((*itr) == (*itt).labelName ) 
				    {
						// we will go get the data that the label points to 
					  ...
					  mc = mc | ((*ittr).machineCode);   // load in the offset
					  continue;
				    }
					else
					{
			  ...
			  else if(isJType(*itr))
			  {
				  ...
				  itt != addressTable.end(); ++itt)
			    {
		          if((*itr) == (*itt).labelName ) 
			      {
				    mc = mc | ((*itt).address);   // load absolute address
				    continue;
				  }
			    }  
			  } 
